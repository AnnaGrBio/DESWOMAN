...
| option      | choices | default     | description     |
| :---        |    :----:   |  :---: | :---  |
| Link to protein database for novelty validation  | Link to a protein dataset       | no link provided   |  This parameter is mandatory for users who decide to perform a protein BLAST search. Here, the user must directly enter the name of the dataset that contains the proteins (and not stop at the folder). Moreover, only **one** dataset is allowed. Therefore, in the case of multiple datasets, all files have to be merged into one single file. If the user does not enter a protein dataset, Step 2 (validation of lack of homology) will not be performed for proteins. If no DNA dataset is provided either, Step 2 is not performed at all.  |
