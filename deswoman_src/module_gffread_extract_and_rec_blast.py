...
from module_transcripts_data_and_threeshold import get_path_to_fasta, get_path_gff
...
def extract_all_prots_genome(pop_ind_name : str, path_to_genome : str, name_intermediate_directory : str) -> None:
    """
    ...
    """
    file_to_assess = name_intermediate_directory + "/old_prot_blast/" + pop_ind_name + "_prot_new.fa"
...
def make_reciprocal_prot_blasts(query_name : str, target_name : str, name_intermediate_directory : str) -> None:
    """
    ...
    link_old_prot_query = name_intermediate_directory + "/old_prot_blast/" + query_name + "_prot_new.fa"
    link_old_prot_target = name_intermediate_directory + "/old_prot_blast/" + target_name + "_prot_new.fa"

    link_prot_query = name_intermediate_directory + "/old_prot_blast/" + query_name + "_prot_new.fa"
    link_prot_target = name_intermediate_directory + "/old_prot_blast/" + target_name + "_prot_new.fa"
    ...
    command1 = "./diamond makedb --in " + link_prot_query + " -d " + output_file_query #query_name
    command2 = "./diamond makedb --in " + link_prot_target + " -d " + output_file_target #target_name
    redirect_comm1 = name_intermediate_directory + "/output_last_diamond_gene_rec_blast_makedb_1.txt"
    redirect_comm2 = name_intermediate_directory + "/output_last_diamond_gene_rec_blast_makedb_2.txt"
    os.system(command1 + ">> " + redirect_comm1)
    os.system(command2 + ">> " + redirect_comm2)
    # Performing reciprocal protein BLAST searches
    name_output_query = name_intermediate_directory + "/old_prot_blast/prot_query_blast_out.txt"
    name_output_target = name_intermediate_directory + "/old_prot_blast/prot_target_blast_out.txt"
    command_blast1 = "./diamond blastp -d " + output_file_target + " -q " + link_prot_query + " -o " + name_output_query # -d was target_name
    command_blast2 = "./diamond blastp -d " + output_file_query + " -q " + link_prot_target + " -o " + name_output_target # -d was query_name
    redirect_blast1 = name_intermediate_directory + "/output_last_diamond_gene_rec_blast_1.txt"
    redirect_blast2 = name_intermediate_directory + "/output_last_diamond_gene_rec_blast_2.txt"
    os.system(command_blast1+ ">> " + redirect_blast1)
    os.system(command_blast2+ ">> " + redirect_blast2)


def make_dico_blast_for_reciprocal_hits(rec_best_hit : str, name_intermediate_directory : str) -> (dict, dict):
    """
    ...
    file_name_query = name_intermediate_directory + "/old_prot_blast/prot_query_blast_out.txt"
    ...
    file_name_target = name_intermediate_directory + "/old_prot_blast/prot_target_blast_out.txt"
    file_blast2 = openFile(file_name_target)
    # Iterating through each line in the second BLAST output file
    for line in file_blast2:
        if line[0] != "#":  # Skipping comment lines
            elts_line = line.split()
            gene_target = elts_line[0]  # Extracting target genes that where here query of the blast
            gene_query = elts_line[1]  # Extracting query genes that were here target of the blast
            # Adding target gene to dictionary under query gene key
