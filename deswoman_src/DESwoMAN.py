import os
import sys
from Main_run_part1 import run_part_1_strat1
from Main_run_part1 import run_part_1_strat2
from Main_run_part2 import run_part_2
from Main_run_part3 import run_part3_strat1
from Main_run_part3 import run_part3_strat2
from module_colors import *
from module_input_checking_strat1 import assess_parameters_strat1
from module_input_checking_strat2 import assess_parameters_strat2
...
def main(strategy : str, link_config : bool|str) -> None:
    ...
    if valid_parameters == True: 
        ...
        if validated_step1 == True: 
            ...
            if dico_variables["link_database_outgroup_prot"] != "" or dico_variables["link_database_outgroup_nucl"] != "": 
                print (BRIGHT_YELLOW + "Running DESwoMAN Part 2." + RESET)
                print ("")
                run_part_2(dico_variables)
                print (YELLOW + "DESwoMAN Part 2 : DONE!" + RESET)
            else:
                print (BRIGHT_YELLOW + "DESwoMAN Part 2 NOT PERFORMED (no database for homology search)" + RESET)
            # Start part 3
            if dico_variables["strategy"] == "1": 
                print ("")
...
def validate_link_config(link : str) -> bool:
    ...
    if os.path.isfile(link) == False:
        print (BRIGHT_RED + "ERROR : The link to config file is not correct : " + RESET + link)
        return False
    else:
        return True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if strategy == "Strategy1" or strategy == "Strategy2":
            ...
            print (BRIGHT_RED + "ERROR : The chosen strategy is not correct : pick Strategy1 or Strategy2" + RESET)
    else:
        print(BRIGHT_RED + "ERROR : No strategy was entered. A strategy must be entered in order to run DESwoMAN" + RESET)
