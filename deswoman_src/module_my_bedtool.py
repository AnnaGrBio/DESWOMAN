import os
from module_sort_gff_gene_order import sort_gff_dic
...
def extract_gff_elts(link_to_gff : str) -> (dict, dict):
    ...
    gene_marker, exon_marker = assess_gff_markers(my_gff)  # i do not use gene marker anymore.
...
def my_bedtool_genes_introns_ordered(link_to_query_genome_gff : str) -> (dict, dict):
    ...
    dico_sorted_chrom_exon_pos = sort_gff_dic(dico_chrom_exon_pos) # from module_sort_gff_gene_order
    dico_sorted_chrom_intron_pos = sort_gff_dic(dico_chrom_intron_pos) # from module_sort_gff_gene_order
    return dico_sorted_chrom_exon_pos, dico_sorted_chrom_intron_pos


def find_intronic_overlap(dico_intron : dict, dict_transcripts_properties : dict, unknown_transcripts : list, antisense_transcript : list, sign_dot : str) -> (list, list):
    """
...
def find_genic_overlap(dico_exon, dict_transcripts_properties : dict, list_accepted_transcript : list, sign_dot : str) -> (list, list):
    """
...
def my_bedtool_extract_overlap(dict_transcripts_properties : dict, dict_gene_transcripts : dict, dico_sorted_chrom_exon_pos : dict, dico_sorted_chrom_intron_pos : dict, name_intermediate_directory : str) -> list:
    """
    ...
    print(BRIGHT_BLUE + "Total number of transcripts above the specified TPM thresshold : " + RESET +str(len(list_accepted_transcript)))
    genic_transcripts, antisense_transcript = find_genic_overlap(dico_sorted_chrom_exon_pos, dict_transcripts_properties, list_accepted_transcript, sign_dot)
    unknown_transcripts = [x for x in list_accepted_transcript if x not in genic_transcripts and x not in antisense_transcript]
    intronic_transcripts, antisense_transcript = find_intronic_overlap(dico_sorted_chrom_intron_pos, dict_transcripts_properties, unknown_transcripts, antisense_transcript, sign_dot)
    intergenic_transcripts = [x for x in unknown_transcripts if x not in antisense_transcript and x not in intronic_transcripts]
    print(BRIGHT_BLUE + "Number of genic transcripts (not denovo) : " + RESET + str(len(genic_transcripts)))
    print(BRIGHT_BLUE + "Number of intergenic transcripts : " + RESET + str(len(intergenic_transcripts)))
    print(BRIGHT_BLUE + "Number of antisense transcripts : " + RESET + str(len(antisense_transcript)) )
    print(BRIGHT_BLUE + "Number of intronic transcripts (in gene frame) : " + RESET + str(len(intronic_transcripts)))
...
def extract_denovo_transcripts(sublist_transcripts_overlap : list, list_choice_denovo : list, dict_transcript_fasta : dict, dict_gene_transcripts : dict, name_output_directory : str) -> (dict, dict, dict, dict):
    """
