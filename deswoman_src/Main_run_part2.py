...
import os
from module_blast_and_diamond import perform_blast_prot_homology_filter, perform_blast_nucl_homology_filter
from module_select_orphans import retrieve_name_hit_prot_blast, retrieve_name_hit_nucl_blast, merge_hit_lists, get_file_size, reshufe_files_in_denovo
...
def run_part_2(dico_variables : dict) -> None:
    ...
    if dico_variables["link_database_outgroup_prot"] != "":
        ...
        perform_blast_prot_homology_filter(name_output_directory, name_intermediate_directory, dico_variables["link_database_outgroup_prot"],dico_variables["parameters_database_prot"]) 
        # if hits were detected, the name of the query denovo with hits are stoded in the list "list_name_to_remove_prot"
        list_name_to_remove_prot = retrieve_name_hit_prot_blast(name_intermediate_directory) 
    ...
    if dico_variables["link_database_outgroup_nucl"] != "":
        ...
        perform_blast_nucl_homology_filter(name_output_directory, name_intermediate_directory, dico_variables["link_database_outgroup_nucl"],dico_variables["parameters_database_nucl"]) 
        # if hits were detected, the name of the query denovo with hits are stoded in the list "list_name_to_remove_nucl"
        list_name_to_remove_nucl = retrieve_name_hit_nucl_blast(name_intermediate_directory) 
    ...
    print (BRIGHT_BLUE + "Number of denovo candidate with a hit : " + RESET + str(len(list_name_to_remove_total)) + " out of " + str(len(dico_size_denovo_prot)))
    new_nb = len(dico_size_denovo_prot) - len(list_name_to_remove_total)
    print (BRIGHT_BLUE + "New number of denovo candidate (without homology) : " + RESET + str(new_nb))
