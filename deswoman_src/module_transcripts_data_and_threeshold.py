...
def associate_gene_and_transcripts(opened_gtf_file : list, min_tpm_asked : int, dict_transcript_fasta_all : dict, te_exclusion : str, unoriented_transc_exclusion : str) -> dict: 
    """
    ...
    list_transcripts_to_exclude = exclude_transcripts(dict_transcript_fasta_all, te_exclusion)
    dico_genes = {}
        ...
        if line[0] != "#":
            ...
            if len(elts_line) > 9:
                ...
                if elts_line[2] == "transcript":   # Note: Handle other applications if needed
                    ...
                    num_column_tpm = detect_tpm_in_file(elts_line) # access the column where the TPM values are
                    TPM = elts_line[num_column_tpm][1:len(elts_line[num_column_tpm])-2]  # Note: Ensure this works for all cases
                    transcript_orientation = elts_line[6]
                    # exclude transcripts with "." as orientation is the user asked for it
                    if unoriented_transc_exclusion == "True" and transcript_orientation == ".":
                    ...
                    if float(TPM) >= float(min_tpm_asked) and transcript_id not in list_transcripts_to_exclude:
...
def extract_transcripts_properties_from_gff(dico_transcripts : dict, elts_line : list) -> None:
    """
    ...
    FPKM = elts_line[nb_coulumn_fpkm][1:len(elts_line[nb_coulumn_fpkm])-2]  # Note: Ensure this works for all cases
    TPM = elts_line[nb_coulumn_tpm][1:len(elts_line[nb_coulumn_tpm])-2]  # Note: Ensure this works for all cases
    coverage = elts_line[nb_coulumn_cov][1:len(elts_line[nb_coulumn_cov])-2]  # Note: Ensure this works for all cases
