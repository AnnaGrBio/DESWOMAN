...
from module_transcripts_data_and_threeshold import get_path_gtf, get_path_to_fasta, get_path_gff, get_path_to_fasta, store_transcriptome_seq, associate_gene_and_transcripts, get_transcripts_properties, get_list_name_query_rep
from module_orfs import my_get_orfs, build_dict_orfs_per_transcript, generate_list_genes_objects, sort_orfs_by_properties
from module_unspliced_orfs import get_dot_orientation, build_dict_unspliced_seqs, rebuild_spliced_orfs, add_stop_codon_to_spliced_orfs
from module_lower_intron import build_unspliced_fasta_seq_lowered_intron
from module_output_part1 import create_new_folder_intermediate, create_unspliced_orfs, create_info_file_orfs, create_denovo_nucl_file, create_denovo_prot_file, create_denovo_transcripts_file
from module_my_bedtool import my_bedtool_genes_introns_ordered, my_bedtool_extract_overlap, extract_denovo_transcripts
from module_reformat_dico_with_IDs import implement_dict_transcripts_properties, implement_dict_gene_status, implement_dict_gene_transcripts_denovo, implement_dict_transcrit_filtered_orfs, implement_dict_pos_unspliced_ORFs_plus_stop, implement_sublist_transcripts_overlap, implement_dict_all_ORFs_filtered_with_stop, implement_dict_transcript_fasta_denovo
...
def run_part_1_strat1(dico_variables : dict) -> bool:
    ...
    dict_gene_transcripts_all = associate_gene_and_transcripts(gtf_file,dico_variables["TPM_threeshold"], dict_transcript_fasta_all, dico_variables["filter_TE"], dico_variables["rm_undir_transc"]) 
    ...
    get_dot_orientation(link_to_query_genome, dict_transcripts_properties, dict_transcript_fasta_all, gtf_file, name_intermediate_directory) 
    ...
    sublist_transcripts_overlap = my_bedtool_extract_overlap(dict_transcripts_properties,dict_gene_transcripts_all, dico_sorted_chrom_exon_pos, dico_sorted_chrom_intron_pos, name_intermediate_directory) 
    ...
    dict_gene_transcripts_denovo,dict_transcript_fasta_denovo,dict_transcrip_status,dict_gene_status = extract_denovo_transcripts(sublist_transcripts_overlap, dico_variables["transcript_overlap"],dict_transcript_fasta_all,dict_gene_transcripts_all, name_output_directory) 

    # Part3 : retrieve orfs from de novo transcripts and reduce them by properties
    # extracts all ORFs from all transcripts
    dict_all_ORFs = my_get_orfs(dico_variables, name_output_directory) 
    if len(dict_all_ORFs) > 0:
        print (BRIGHT_BLUE + "Total number of correct sense ORFs in denovo transcripts : " + RESET + str(len(dict_all_ORFs)))
        ...
        list_gene_object = generate_list_genes_objects(dict_gene_transcripts_denovo,dict_transcrit_all_orfs,dict_all_ORFs,dict_transcript_fasta_denovo) 
        # goes through each gene object created before and retreive the ORFs we want to keep from all the splice variants. 
        dict_transcrit_filtered_orfs,dict_all_ORFs_filtered = sort_orfs_by_properties(dico_variables["filter_genic"], list_gene_object, dico_variables["ORFs_choice"],dict_all_ORFs, dict_gene_status)    
        print (BRIGHT_BLUE + "Total number of correct sense ORFs in denovo transcripts filtered : " + RESET + str(len(dict_all_ORFs_filtered)))
        ...
        dict_pos_unspliced_ORFs_plus_stop = rebuild_spliced_orfs(link_to_query_genome, dict_gene_exon_pos, dict_all_ORFs_filtered, name_intermediate_directory) 
        ...
        print (BRIGHT_BLUE + "Total number of filtered ORFs correctly detected in genomes : " + RESET + str(len(dict_all_ORFs_filtered_with_stop)))
        ...
        create_unspliced_orfs(name_output_directory, dict_unspliced_orfs_fasta_lowered_introns) 
        create_info_file_orfs(name_output_directory, dict_transcripts_properties, dict_gene_status, dict_gene_transcripts_denovo, dict_transcrit_filtered_orfs,dict_pos_unspliced_ORFs_plus_stop,sublist_transcripts_overlap) 
        ...
        create_denovo_transcripts_file(name_output_directory, dict_transcript_fasta_denovo)
        validated_step = True
...
def run_part_1_strat2(dico_variables : dict) -> bool:
    ...
    for ID_transcriptome in list_query_name_transc:
        print ("")
        print (YELLOW + "-----------" + RESET)
        print (BRIGHT_BLUE + "Name query : "  + RESET + BRIGHT_CYAN + ID_transcriptome + RESET)
    ...
    if len(dict_gene_transcripts_denovo_total) > 0:
        # generate all output files (fasta + info file) of denovo candidates filtered after step 1.
        create_info_file_orfs(name_output_directory, dict_transcripts_properties_total, dict_gene_status_total, dict_gene_transcripts_denovo_total, dict_transcrit_filtered_orfs_total,dict_pos_unspliced_ORFs_plus_stop_total,sublist_transcripts_overlap_total) 
        create_denovo_nucl_file(name_output_directory, dict_all_ORFs_filtered_with_stop_total) 
        create_denovo_prot_file(name_output_directory) 
        create_denovo_transcripts_file(name_output_directory, dict_transcript_fasta_denovo_total) 
        validated_step = True
    else:
        print (BRIGHT_MAGENTA + "No denovo transcript found ! " + RESET)
    return validated_step
