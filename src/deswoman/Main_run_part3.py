...
from module_generate_outgroup_dict import get_dot_special_case_dic, create_outgroup_dict_strategy1, create_outgroup_dict_strategy1
from module_blast_and_diamond import perform_blast_to_genome, get_transcription_hits, perform_blast_prot_transcripts
from module_store_all_nc_hits import make_dico_len_unspliced_denovo, extract_best_blast_outputs, extract_all_blast_outputs, store_de_novo_informations
from module_gffread_extract_and_rec_blast import extract_all_prots_genome, make_reciprocal_prot_blasts, make_dico_blast_for_reciprocal_hits
from module_sort_gff_gene_order import store_genome_informations, sort_gff_dic
from module_synteny import search_all_denovo_hits
from module_prepare_global_alignment import build_dico_seq_denovo_filtered_candidates, build_dico_seq_NcHomologs
from module_global_alignment_properties import get_transcription_indication, main_alignment_function, create_final_output_file
from module_handle_strat3_hits import build_target_and_query_prot_list, make_dico_name_denovo_strat2, handle_blast_inputs_transc, build_final_file_strat2_step3
from module_remove_extra_files import remove_intermediates
...
def run_part3_strat1(dico_variables : dict) -> None:
    ...
    for pop_species_name in dico_target_species_lines:
        print ("")
        print (YELLOW + "-----------" + RESET)
        print (BRIGHT_BLUE + "Name target : "  + RESET + BRIGHT_CYAN + pop_species_name + RESET)
        ...
        check_empty = perform_blast_to_genome(pop_species_name, dico_target_species_lines[pop_species_name]["genome_fasta"], name_output_directory) 
        print (BRIGHT_BLUE + "Homology search of denovo candidate against target genome performed" + RESET)
        if check_empty != 0:
            # store best blast hits if no synteny critaria
            if dico_variables["synteny_window"] == 0:
                dico_denovo_best_hit = extract_best_blast_outputs(pop_species_name, name_intermediate_directory)
            else:
                # store all blast hits (dico[namedenoo] : [namehit1, namehit2, etc])
                dict_my_blast_selected_outputs = extract_all_blast_outputs(pop_species_name, name_intermediate_directory) 
                # extracts necessary informations of all denovo candidates in info file
                dico_denovo_informations = store_de_novo_informations(name_output_directory) 
                # extract outgroup proteins and store them in the "old_prot" directory.
                extract_all_prots_genome(pop_species_name, dico_variables["path_to_genome_repository"], name_intermediate_directory) 
                # same for prots of query genome
                extract_all_prots_genome(dico_variables["query"], dico_variables["path_to_genome_repository"], name_intermediate_directory)
                print (BRIGHT_BLUE + "Proteins extracted" + RESET)
                
                # perform reciprocal blast between query and target annotated proteins
                make_reciprocal_prot_blasts(dico_variables["query"], pop_species_name, name_intermediate_directory)
                # Store hits from query against target and target against query in 2 dicts.
                dico_blast1, dico_blast2 = make_dico_blast_for_reciprocal_hits(dico_variables["rec_best_hit"], name_intermediate_directory) 
                print (BRIGHT_BLUE + "Homology detection between annotated genes of query and target genomes performed" + RESET)
            ...
            if len(dico_denovo_best_hit) > 0:   
                ...
                dico_denovo_seq_intron = build_dico_seq_denovo_filtered_candidates(dico_denovo_best_hit, name_output_directory) 
                ...
                dico_target_transcript_coordinate = get_transcription_indication(dico_target_species_lines, pop_species_name, dico_dot_special_case, dico_variables["rm_undir_transc"]) 
                ...
                dico_target_transcript_blast_hits = get_transcription_hits(dico_target_species_lines, pop_species_name, name_output_directory)  
                # dico_format_all_data is implemented for each pop and each de novo, with all mutations observed.
                dico_format_all_data = main_alignment_function(dico_variables["start_codon"],pop_species_name, dico_name_size_denovo, dico_denovo_best_hit, dico_denovo_seq_intron, dico_NcHit, dico_target_transcript_coordinate, dico_target_transcript_blast_hits, dico_variables["premature_stop"], dico_format_all_data)  
    if len(dico_format_all_data) > 0:
        # create final file.
        create_final_output_file(dico_format_all_data, name_output_directory, name_intermediate_directory) 
    else:
...
def run_part3_strat2(dico_variables : dict) -> None:
    ...
    handle_blast_inputs_transc(dico_orthogroups, dico_name_size_denovo, name_intermediate_directory, name_output_directory) 
    build_final_file_strat2_step3(dico_orthogroups, name_intermediate_directory, name_output_directory)
    remove_intermediates(name_intermediate_directory)
