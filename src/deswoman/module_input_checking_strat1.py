...
def validate_presence_of_mandatory_parameters_strat1(
...
def validate_presence_query_transcriptome_strat1(
...
def validate_presence_query_name_strat1(dico_variables: dict, RUN_PYTHON: bool) -> bool:
    ...
    presence_fasta_genome = validate_presence_query_genome_strat1(
    ...
    presence_fasta_transcriptome = validate_presence_query_transcriptome_strat1(
        query_name, path_to_transcriptome
    )
    presence_fasta_gtf = validate_presence_query_gtf_strat1(
        query_name, path_to_transcriptome
    )
    ...
    if presence_fasta_transcriptome == False:
        ...
        print(
            BRIGHT_RED
            + "ERROR : Query transcriptome "
            + name_fasta_transcriptome
            + " (or .fa|.fna) not present in transcriptome folder"
            + RESET
        )
    if presence_fasta_gtf == False:
        ...
        print(
            BRIGHT_RED
            + "ERROR : Query transcriptome annotation "
            + name_gtf_transcriptome
            + " not present in transcriptome folder"
            + RESET
        )
    return RUN_PYTHON
...
def search_liste_name_target_genome_strat1(
    query_name: str, path_genome_folder: str
...
def validate_target_genomes_strat1(
    dico_variables: dict, RUN_PYTHON: bool
) -> tuple[bool, list]:
    """
    ...
    """
    path_to_genome = dico_variables["path_to_genome_repository"]
    list_target_name, liste_target_genome, liste_target_genome_annotation = (
        search_liste_name_target_genome_strat1(dico_variables["query"], path_to_genome)
    )
    ...
    for name in list_target_name:
        ...
        if genome_p == False:
            ...
            print(
                ...
                + "ERROR : target "
                + name
                + " has genome annotation file but no genome"
                + RESET
            )
        if annotation_p == False:
            ...
            print(
                ...
                + "ERROR : target "
                + name
                + " has genome file but no annotation"
                + RESET
            )
        if genome_valid == False:
            ...
            print(
                ...
                + "ERROR : target "
                + name
                + " genome seems to NOT be in FASTA format"
                + RESET
            )
        if annotation_valid == False:
            ...
            print(
                ...
                + "ERROR : target "
                + name
                + " genome annotation format (GFF/GFF3) seems to be invalid"
                + RESET
            )
            print(" ")
...
def search_liste_name_target_transcriptome_strat1(
...
def validate_target_transcriptomes_strat1(
    dico_variables: dict, RUN_PYTHON: bool, list_genome_name: list
) -> bool:
    """
    ...
    (
        ...
        liste_target_transcriptome,
        liste_target_transcriptome_annotation,
    ) = search_liste_name_target_transcriptome_strat1(
    ...
    for name_transcriptome in list_transcriptome_name:
        if name_transcriptome not in list_genome_name:
            print(
                ...
                + "WARNING : "
                + name_transcriptome
                + " transcriptome does not correspond to a genome"
                + RESET
            )
            print(
                ORANGE
    ...
    for name in list_genome_name:
        ...
        if name_is_present == False:
            print(
                ORANGE + "WARNING : " + name + " does not have a transcriptome" + RESET
            )
            print(
                ORANGE
                + "---> "
                + name
                + " outgroup homologs will not be analysed for transcription"
                + RESET
            )
        ...
        if annotation_p == False and name_is_present == True:
            print(
                ...
                + "WARNING : target "
                + name
                + " has transcriptome but no corresponding annotation"
                + RESET
            )
            print(
                ORANGE
...
def search_name_in_folder_strat1(
...
def validate_file_content_query_strat1(dico_variables: dict, RUN_PYTHON: bool) -> bool:
    ...
    """
    # extract query files
    fasta_genome = search_name_in_folder_strat1(
        dico_variables["path_to_genome_repository"],
        dico_variables["query"],
        ["fasta", "fna", "fa"],
    )
    gff_genome = search_name_in_folder_strat1(
        dico_variables["path_to_genome_repository"],
        dico_variables["query"],
        ["gff", "gff3"],
    )
    fasta_transcriptome = search_name_in_folder_strat1(
        dico_variables["path_to_transcriptome_repository"],
        dico_variables["query"],
        ["fasta", "fna", "fa"],
    )
    gtf_transcriptome = search_name_in_folder_strat1(
        dico_variables["path_to_transcriptome_repository"],
        dico_variables["query"],
        ["gtf"],
    )
    ...
    if fasta_transcriptome_correct == False:
        ...
        print(
            BRIGHT_RED
            + "ERROR : The query transcriptome does not seem to be in FASTA format"
            + RESET
        )
    ...
    if gtf_transcriptome_correct == False:
        ...
        print(
            BRIGHT_RED
            + "ERROR : The query transcriptome annotation does not seem to be in correct GTF format"
            + RESET
        )
    return RUN_PYTHON
...
def display_parameters_strat1(dico_variables: dict) -> None:
    ...
    print(
        ...
        + RESET
        + str(dico_variables["rec_best_hit"])
    )
    print(
        BRIGHT_GREEN
        + "Synteny window : "
        + RESET
        + str(dico_variables["synteny_window"])
    )
    print(
        BRIGHT_GREEN
        + "Search premature stop in (sequence percentage) : "
        + RESET
        + str(dico_variables["premature_stop"])
        + " %"
    )
    print(" ")
    if dico_variables["link_database_outgroup_prot"] != "":
        print(
            BRIGHT_GREEN
            + "Validation of NO PROTEIN homology to neORF will be performed against : "
            + RESET
        )
    ...
    if dico_variables["link_database_outgroup_nucl"] != "":
        print(
            BRIGHT_GREEN
            + "Validation of NO DNA homology to neORF will be performed against : "
            + RESET
        )
...
def assess_parameters_strat1(link_config: str) -> tuple[bool, dict]:
    """
    ...
    if RUN_PYTHON == True:
        ...
        RUN_PYTHON = validate_presence_of_mandatory_parameters_strat1(
    ...
    if RUN_PYTHON == True:
        ...
        RUN_PYTHON, list_target_name = validate_target_genomes_strat1(
            dico_variables, RUN_PYTHON
        )
    if RUN_PYTHON == True:
    ...
    if RUN_PYTHON == True:
        ...
        RUN_PYTHON = validate_target_transcriptomes_strat1(
            dico_variables, RUN_PYTHON, list_target_name
        )
...
# - "start_codon" : the chosen start codon
