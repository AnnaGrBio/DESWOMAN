...
def search_surrounding_gene(
    start: int, stop: int, chrom: str, dico_gff_focal_sorted: dict, window: int
) -> list:
    """
    ...
    if chrom in dico_gff_focal_sorted:
        ...
        if (
            len(matrix) >= window * 2
        ):  # means if the chromosome contains more genes that 2 times the window
            matrix_start = []
            dico_associated = {}
            # matrix_start implements the start of all genes in the chrom
            for sublist_gene_info in matrix:
                matrix_start.append(
                    int(sublist_gene_info[0])
                )  # the matrix implements the start position of the gene
...
def search_overlapping_gene(
    start: int, stop: int, chrom: str, dico_gff_focal_sorted: dict
) -> list:
    """
...
def determine_neigboor_function(
    denovo_genomic_infos: list, dico_gff_focal_sorted: dict, window: int
) -> list:
    """
    ...
    if denovo_genomic_infos[0] == "intergenic_transcripts":
        list_neighboor = search_surrounding_gene(
            start, stop, chrom, dico_gff_focal_sorted, window
        )
    else:
        list_neighboor = search_overlapping_gene(
            start, stop, chrom, dico_gff_focal_sorted
        )
    return list_neighboor


def get_synteny_score(
        ...
        if gene_neighboor_denovo in dico_blastA:
            ...
            for homologs_gene_neigboor_denovo in list_homologs_gene_neigboor_denovo:
                ...
                if homologs_gene_neigboor_denovo in new_list_neighboor_hit:
                    if (
                        homologs_gene_neigboor_denovo in dico_blastB
                        and gene_neighboor_denovo
                        in dico_blastB[homologs_gene_neigboor_denovo]
                    ):
                        nb_reciprocal_hit += 1
...
def search_all_denovo_hits(
    ...
    for denovo in dico_denovo_informations:
        ...
        if denovo in dict_my_blast_selected_outputs:
            ...
            list_neighboor_denovo = determine_neigboor_function(
                denovo_genomic_infos, dico_gff_focal_sorted, window
            )
            best_hit = []
            best_score = 0
            for hit in list_denovo_hits:
                ...
                list_neighboor_hit = determine_neigboor_function(
                    hit_genomic_infos, dico_gff_outgroup_sorted, window
                )
                nb_reciprocal_hit = get_synteny_score(
                    list_neighboor_denovo, list_neighboor_hit, dico_blast1, dico_blast2
                )
    ...
    print(
        BRIGHT_BLUE
        + "Number of neORF candidate : "
        + RESET
        + str(len(dico_denovo_informations))
    )
    print(
        BRIGHT_BLUE
        + "Number of neORF candidate with syntenic hits : "
        + RESET
        + str(len(dico_denovo_best_hit))
    )
    print(
        BRIGHT_BLUE
        + "Percentage of candidate neORF with syntenic hits : "
        + RESET
        + str(avg)
    )
