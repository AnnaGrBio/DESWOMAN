...
from module_graphical_interface_strat2 import my_graphical_interface_strategy2
...
def validate_presence_of_mandatory_parameters_strat2(dico_variables : dict, RUN_PYTHON : bool) -> bool:
    """
...
def validate_presence_query_transcriptome_strat2(query_name : str, path_rep : str) -> bool:
    """
...
def validate_presence_query_name_strat2(dico_variables : dict, RUN_PYTHON : bool) -> bool:
    ...
    presence_fasta_genome = validate_presence_query_genome_strat2(query_name, path_to_genome)
...
def search_liste_name_target_transcriptome_strat2(query_name : str, path_transcriptome_folder : str) -> (list, list, list):
    """
...
def validate_target_transcriptomes_strat2(dico_variables : dict, RUN_PYTHON : bool) -> (bool, list):
    """
    ...
    """
    path_to_transcriptome = dico_variables["path_to_transcriptome_repository"]
    list_transcriptome_name, liste_target_transcriptome, liste_target_transcriptome_annotation = search_liste_name_target_transcriptome_strat2(dico_variables["query"], path_to_transcriptome)
    ...
    for name in list_transcriptome_name:
        ...
        if transcriptome_p == False:
            print (ORANGE + "WARNING : query " + name + " has transcriptome annotation file but no transcriptome" + RESET)
            print (ORANGE + "---> " + name + " will not be analysed for transcription" + RESET)
            liste_name_to_remove.append(name)
        if annotation_p == False:
            print (ORANGE + "WARNING : query " + name + " has transcriptome file but no annotation" + RESET)
            print (ORANGE + "---> " + name + " will not be analysed for transcription" + RESET)
            liste_name_to_remove.append(name)
...
def search_name_in_folder_strat2(path_folder : str, name_query : str, list_format : list) -> str:
    """
...
def validate_file_content_query_strat2(dico_variables : dict, RUN_PYTHON : bool) -> bool:
    ...
    """
    fasta_genome = search_name_in_folder_strat2(dico_variables["path_to_genome_repository"], dico_variables["query"], ["fasta", "fna", "fa"])
    gff_genome = search_name_in_folder_strat2(dico_variables["path_to_genome_repository"], dico_variables["query"], ["gff", "gff3"])
...
def display_parameters_strat2(dico_variables : dict) -> None:
    ...
    if dico_variables["link_database_outgroup_prot"] != "":
        print (BRIGHT_GREEN + "Validation of NO PROTEIN homology will be performed against : " + RESET)
    ...
    if dico_variables["link_database_outgroup_nucl"] != "":
        print (BRIGHT_GREEN + "Validation of NO DNA homology will be performed against : " + RESET)
...
def assess_parameters_strat2(link_config : str) -> (bool, dict):
    ...
    if RUN_PYTHON == True:
        ...
        RUN_PYTHON = validate_presence_of_mandatory_parameters_strat2(dico_variables, RUN_PYTHON)
    if RUN_PYTHON == True:
    ...
    if RUN_PYTHON == True:
        ...
        RUN_PYTHON, list_transcriptome_name = validate_target_transcriptomes_strat2(dico_variables, RUN_PYTHON)
    if RUN_PYTHON == True:
        # a minimum of 2 query transcriptomes are required for strategy 2
        if len(list_transcriptome_name) < 2:
            RUN_PYTHON = False
            print (BRIGHT_RED + "ERROR : The transcriptome folder contains 0 or 1 valid transcriptome (min required : 2)" + RESET)
    if RUN_PYTHON == True:
